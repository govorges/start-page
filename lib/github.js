...
// Weekly commit counts for the default branch. GitHub answers 202 while it computes them, which for some
// repositories (new ones especially) can go on for hours; activity() then counts recent commits itself.
...
async function activity(cfg) {
  ...
  const vars = { since: new Date(Date.parse(utcDay(Date.now())) - (ACTIVITY_DAYS - 1) * 86400e3).toISOString() }, defs = ['$since: GitTimestamp!'], fields = [];
  repos.forEach((full, i) => {
    const [o, n] = full.split('/');
    vars['o' + i] = o; vars['n' + i] = n;
  ...
  const list = repos.map((full, i) => {
    ...
    if (!r) return null;
    const commits = zeros();
    // GitHub's weekly statistics when they're ready; otherwise the latest 100 commits on the default branch
    // (all of them in 180 days for most repositories, a partial count for busy ones).
    const history = r.defaultBranchRef && r.defaultBranchRef.target && r.defaultBranchRef.target.history;
    let commitsStatus = Array.isArray(stats[i]) ? 'ok' : stats[i], commitCoverage = ACTIVITY_DAYS;
    if (Array.isArray(stats[i])) {
      for (const w of stats[i]) w.days.forEach((n, day) => add(commits, (w.week + day * 86400) * 1000, n));
    } else if (history) {
      history.nodes.forEach(c => add(commits, c.committedDate));
      commitCoverage = history.nodes.length < 100 ? ACTIVITY_DAYS : Math.min(ACTIVITY_DAYS, daysAgo(history.nodes[history.nodes.length - 1].committedDate));
      commitsStatus = 'ok';
    }
    ...
    return {
      ...
      coverage: { commits: commitCoverage, prs: coverage(r.pullRequests.nodes), issues: coverage(r.issues.nodes) },
