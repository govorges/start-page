...
function gitProblem() {
  ...
  if (changed) {
    // Porcelain lines are a status code, a space and the path (the first line's leading space was trimmed).
    const files = changed.split('\n').map(l => l.trim().replace(/^\S+\s+/, '')).filter(Boolean);
    const list = files.length > 3 ? `${files.slice(0, 3).join(', ')} and ${files.length - 3} more` : files.join(files.length === 2 ? ' and ' : ', ');
    return `${list} ${files.length === 1 ? 'has changes that aren’t' : 'have changes that aren’t'} committed. Commit or discard them first (git restore <file> discards), so git pull doesn’t mix them with the update.`;
  }
  return null;
}
