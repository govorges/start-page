...
async function ensureAppExe(nodePath = process.execPath, env = 'live') {
  ...
  if (isAppExe(nodePath)) {
    ...
    if (!nodePath || !fs.existsSync(nodePath) || isAppExe(nodePath)) return fs.existsSync(exePath) ? usable(exePath, process.execPath) : process.execPath;
  }
  try {
    ...
    if (have && fs.existsSync(exePath) && ['nodePath', 'nodeSize', 'icon'].every(k => have[k] === want[k])) return usable(exePath, nodePath);
    ...
    return usable(exePath, nodePath);
  } catch (e) {
    console.warn(`Couldn’t prepare ${v.exe}, so Node will be used directly:`, e.message);
    return fs.existsSync(exePath) ? usable(exePath, nodePath) : nodePath;
  }
}

// Changing Node's icon and name removes its digital signature, and Windows 11's Smart App Control (or
// some antivirus programs) can block unsigned programs, deciding file by file. So check that Windows
// lets this one run; if not, use Node itself, which is signed and works the same (Task Manager just
// lists it as Node.js).
function usable(exePath, nodePath) {
  const r = spawnSync(exePath, ['-e', '0'], { stdio: 'ignore', windowsHide: true, timeout: 20000 });
  if (!r.error && r.status === 0) return exePath;
  console.warn(`Windows didn’t allow ${path.basename(exePath)} to run (Smart App Control or antivirus), so Node will be used directly.`);
  return nodePath;
}
