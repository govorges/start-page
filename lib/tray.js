...
async function start(onCommand, env = 'live') {
  ...
  if (!exe) return null;
  const blocked = e => console.warn('Couldn’t show the tray icon (Windows may have blocked StartPageTray.exe, for example with Smart App Control):', e.message);
  let child;
  try { child = spawn(exe, [iconPath(env)], { stdio: ['pipe', 'pipe', 'ignore'], windowsHide: true }); }
  catch (e) { blocked(e); return null; } // a program Windows blocks makes spawn throw
  child.on('error', blocked);
